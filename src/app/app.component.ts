...
import { loadFont, makeStrips, Point2D, Strip } from './shape';
...
export class AppComponent implements OnInit {
  ...
  // private loadFont() {
  //   opentype.load('assets/courier.ttf', function (err, font) {
  //     if (err) {
  ...
  //       const path = font.getPath('A', 0, 150, 72);
  //       console.log('path:', path);

  //       for (let i = 0; i < path.commands.length; i++) {
  //         const element = path.commands[i];
   ...
   */
  calculateQuadraticBezierPoint(t: number, p0: Point2D, p1: Point2D, p2: Point2D) {
    const mt = 1 - t; // (1-t)
 ...
 * @returns {object} The interpolated point {x, y}.
 */
  calculateCubicBezierPoint(t: number, p0: Point2D, p1: Point2D, p2: Point2D, p3: Point2D) {
   ...
   */
  generateQuadraticBezierCurveVertices(p0: Point2D, p1: Point2D, p2: Point2D, segments: number) {
    const vertices = [];
    for (let i = 0; i <= segments; i++) {
      const t = i / segments;
      const point = this.calculateQuadraticBezierPoint(t, p0, p1, p2);
 ...
 * @returns {Float32Array} An array of x, y coordinates for the curve.
 */
  generateCubicBezierCurveVertices(p0: Point2D, p1: Point2D, p2: Point2D, p3: Point2D, segments: number) {
    const vertices = [];
    for (let i = 0; i <= segments; i++) {
      const t = i / segments;
      const point = this.calculateCubicBezierPoint(t, p0, p1, p2, p3);
      vertices.push(point.x, point.y);
    }
    return new Float32Array(vertices);
  }
  ...
  //   // Define the control points for our quadratic Bezier curve in clip space
  //   const p0 = { x: -0.8, y: -0.8 }; // Start point
  //   const p1 = { x: -0.4, y: 0.8 }; // Control point (pulls the curve upwards)
  //   const p2 = { x: 0.4, y: -0.8 };
  //   const p3 = { x: 0.8, y: -0.8 }; // End point  

  //   // Generate the vertices for the Bezier curve
  //   const segments = 100; // Number of line segments to approximate the curve
  //   // const bezierVertices = this.generateQuadraticBezierCurveVertices(p0, p1, p2, segments);
  //   const bezierVertices = this.generateCubicBezierCurveVertices(p0, p1, p2, p3, segments);
  ...
  //   // Draw the vertices. We draw all generated vertices.
  //   renderPassEncoder.draw(bezierVertices.length / 2); // Draw all points as a line strip
